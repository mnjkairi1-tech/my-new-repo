...
import { contentCreationToolData } from '@/lib/content-creation-tool-data';
import type { Tool } from '@/lib/types';
import { Card, CardContent, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { ExternalLink, ArrowLeft, Star, Share2, Filter, Feather } from 'lucide-react';
...
export default function ContentCreationPage() {
    ...
    const filteredToolData = useMemo(() => {
        const validCategories = contentCreationToolData.filter(Boolean);
        if (priceFilter === 'All') {
        ...
        <div className="bg-background min-h-screen flex flex-col items-center justify-start font-body relative">
            ...
            <main className="relative z-10 w-full max-w-sm flex-1 flex flex-col min-h-0 mt-6">
                <div className="flex-grow overflow-y-auto no-scrollbar p-4 space-y-8">
                    ...
                    if (!category || !category.tools || category.tools.length === 0) return null;

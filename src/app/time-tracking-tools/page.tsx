...
import { 
    ArrowLeft, ExternalLink, Star, Share2, Clock, Filter, X, Plus, Loader2
} from 'lucide-react';
...
import { type Tool, timeTrackingToolData } from '@/lib/time-tracking-tools-data';
...
export default function TimeTrackingToolsPage() {
    ...
    const handleAddTool = async () => {
        ...
        try {
            ...
            const toolData = {
                ...
                description: info.toolDescription || 'Time Tracking Tool',
                url: url,
                image: `https://www.google.com/s2/favicons?sz=128&domain=${new URL(url).hostname}`,
                dataAiHint: (info.toolName || 'time tracking').toLowerCase(),
    ...
    const filteredToolData = useMemo(() => {
        ...
        return timeTrackingToolData.map(category => {
        ...
        <header className="flex items-center justify-between gap-4">
            <div className='flex items-center gap-4'>
                ...
                <div className='flex items-center gap-2'>
                    <Clock className={cn("w-6 h-6", isMidnight ? "text-white" : "text-foreground")} />

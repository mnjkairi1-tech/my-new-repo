...
import { 
    ArrowLeft, ExternalLink, Star, Share2, Filter, Workflow, X, Plus, Loader2
} from 'lucide-react';
...
import { type Tool, automationWorkflowToolData } from '@/lib/automation-workflow-tools-data';
...
export default function AutomationWorkflowToolsPage() {
    ...
    const handleAddTool = async () => {
        ...
        try {
            ...
            const toolData = {
                ...
                description: info.toolDescription || 'AI Automation Tool',
                url: url,
                image: `https://www.google.com/s2/favicons?sz=128&domain=${new URL(url).hostname}`,
                dataAiHint: (info.toolName || 'automation').toLowerCase(),
    ...
    const filteredToolData = useMemo(() => {
        ...
        return automationWorkflowToolData.map(category => {
        ...
        <header className="flex items-center justify-between gap-4">
            <div className='flex items-center gap-4'>
                ...
                <div className='flex items-center gap-2'>
                    <Workflow className={cn("w-6 h-6", isMidnight ? "text-white" : "text-foreground")} />

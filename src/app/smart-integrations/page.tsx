...
import { 
    ArrowLeft, ExternalLink, Star, Share2, Filter, Zap, X, Plus, Loader2
} from 'lucide-react';
...
import { type Tool, smartIntegrationsToolData } from '@/lib/smart-integrations-data';
...
export default function SmartIntegrationsPage() {
    ...
    const handleAddTool = async () => {
        ...
        try {
            ...
            const toolData = {
                ...
                description: info.toolDescription || 'Smart Integration Tool',
                url: url,
                image: `https://www.google.com/s2/favicons?sz=128&domain=${new URL(url).hostname}`,
                dataAiHint: (info.toolName || 'integration').toLowerCase(),
    ...
    const filteredToolData = useMemo(() => {
        ...
        return smartIntegrationsToolData.map(category => {
        ...
        <header className="flex items-center justify-between gap-4">
            <div className='flex items-center gap-4'>
                ...
                <div className='flex items-center gap-2'>
                    <Zap className={cn("w-6 h-6", isMidnight ? "text-white" : "text-foreground")} />
